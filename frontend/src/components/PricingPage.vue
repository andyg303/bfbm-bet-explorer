...
<template>
  <div class="min-h-screen bg-[#0b0f1a] text-gray-200 transition-colors duration-200">
    <!-- Nav -->
      ...
      <div class="max-w-5xl mx-auto px-4 sm:px-6">
        <div class="flex h-16 items-center justify-between">
          <button @click="$emit('navigate', 'landing')" class="flex items-center gap-3 hover:opacity-80 transition-opacity">
            ...
            <span class="text-lg font-bold text-white tracking-tight">BFBM<span class="text-teal-400">Explorer</span></span>
          </button>
          <div class="flex items-center gap-3">
            <button @click="toggleDark" class="p-2 rounded-lg text-gray-500 hover:text-gray-300 hover:bg-gray-800/50 transition-colors">
            ...
            <template v-else>
              <button @click="$emit('navigate', 'login')" class="px-4 py-2 text-sm font-medium text-gray-400 hover:text-white transition-colors">Sign In</button>
            </template>
          </div>
        </div>
      </div>
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 py-16 sm:py-24">
      ...
      <div class="text-center mb-14">
        <h1 class="text-3xl sm:text-4xl font-bold text-white">Simple, transparent pricing</h1>
      ...
      <!-- Error -->
      <div v-if="error" class="mb-8 max-w-lg mx-auto p-4 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 text-sm text-center">{{ error }}</div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
        <div
          ...
          :class="plan.popular ? 'accent-stripe' : ''"
        >
          <div class="h-full glass-card !rounded-2xl" :class="plan.popular ? '!border-teal-500/30' : ''">
            <!-- Popular badge -->
            <div v-if="plan.popular" class="absolute -top-0 right-6 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-teal-400 bg-teal-500/10 border border-teal-500/20 rounded-b-lg z-10">Best Value</div>

            <div class="p-8">
              <h3 class="text-lg font-semibold text-white">{{ plan.name }}</h3>

              <div class="mt-4 flex items-baseline gap-2">
                <span class="text-4xl sm:text-5xl font-bold text-white font-mono">{{ plan.price }}</span>
                <div class="text-sm text-gray-500">
                  <div>one-time</div>
                  <div class="font-medium text-teal-400">{{ plan.perMonth }}/mo</div>
                </div>
              </div>

              <div v-if="plan.savings" class="mt-3">
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">{{ plan.savings }}</span>
              </div>

              <ul class="mt-6 space-y-3">
                <li v-for="feat in plan.features" :key="feat" class="flex items-start gap-2.5 text-sm text-gray-400">
                  <svg class="w-5 h-5 text-teal-400 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                  {{ feat }}
                </li>
              </ul>
              ...
              <button
                ...
                :class="plan.popular ? 'btn-glow' : 'text-teal-400 bg-teal-500/10 hover:bg-teal-500/20 border border-teal-500/20'"
              >
